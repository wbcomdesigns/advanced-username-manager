 ...
 * @version 1.6.15
 */
class ADVANCED_USERNAME_MANAGER_PLUGIN_UPDATER {
		private $api_url = '';
	...
	public function show_update_notification( $file, $plugin ) {
		...
		printf(
			...
			esc_html__( 'There is a new version of %1$s available.', 'advanced-username-manager' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'advanced-username-manager' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				esc_html__( '%1$sView version %2$s details%3$s.', 'advanced-username-manager' ),
			...
			printf(
				/* translators: 
				 * 1: Opening anchor tag for "View version details" link 
				 * 2: Plugin version number 
				 * 3: Closing anchor tag for "View version details" link 
				 * 4: Opening anchor tag for "update now" link 
				 * 5: Closing anchor tag for "update now" link 
				 */
				esc_html__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'advanced-username-manager' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'advanced-username-manager' ),
				'</a>'
			);
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'advanced-username-manager' ), esc_html__( 'Error', 'advanced-username-manager' ), array( 'response' => 403 ) );
		}
